...
export const calendarDates = sqliteTable(
  ...
  (table) => {
    return {
      ...
      pk0: primaryKey({
        columns: [table.date, table.serviceId],
        name: "calendar_dates_date_service_id_pk",
      }),
    };
  },
);
...
export const fareLegRules = sqliteTable(
  "fare_leg_rules",
  {
  ...
  (table) => {
    return {
      pk0: primaryKey({
        ...
        name: "fare_leg_rules_fare_product_id_from_area_id_network_id_to_area_id_pk",
      }),
    };
  },
);

export const fareProducts = sqliteTable(
  "fare_products",
  {
    fareProductId: numeric("fare_product_id").notNull(),
  ...
  (table) => {
    return {
      pk0: primaryKey({
        columns: [table.fareMediaId, table.fareProductId],
        name: "fare_products_fare_media_id_fare_product_id_pk",
      }),
    };
  },
);
...
export const fareTransferRules = sqliteTable(
  "fare_transfer_rules",
  {
    ...
    return {
      pk0: primaryKey({
        columns: [
          table.durationLimit,
          table.fareProductId,
          table.fromLegGroupId,
          table.toLegGroupId,
          table.transferCount,
        ],
        name: "fare_transfer_rules_duration_limit_fare_product_id_from_leg_group_id_to_leg_group_id_transfer_count_pk",
      }),
    };
  },
);
...
export const frequencies = sqliteTable(
  ...
  (table) => {
    return {
      pk0: primaryKey({
        columns: [table.startTime, table.tripId],
        name: "frequencies_start_time_trip_id_pk",
      }),
    };
  },
);
  ...
  (table) => {
    return {
      pk0: primaryKey({
        ...
        name: "shapes_shape_id_shape_pt_sequence_pk",
      }),
    };
  },
);
...
export const stopTimes = sqliteTable(
  ...
  (table) => {
    return {
      ...
      pk0: primaryKey({
        columns: [table.stopSequence, table.tripId],
        name: "stop_times_stop_sequence_trip_id_pk",
      }),
    };
  },
);
  ...
  (table) => {
    return {
      pk0: primaryKey({
        ...
        name: "transfers_from_route_id_from_stop_id_from_trip_id_to_route_id_to_stop_id_to_trip_id_pk",
      }),
    };
  },
);
  ...
  (table) => {
    return {
      pk0: primaryKey({
        ...
        name: "translations_field_name_field_value_language_record_id_record_sub_id_table_name_pk",
      }),
    };
  },
);
...
export const timetableStopOrder = sqliteTable(
...
export const timetableNotesReferences = sqliteTable(
  "timetable_notes_references",
  {
  ...
  (table) => {
    return {
      idxTimetableNotesReferencesStopSequence: index(
        "idx_timetable_notes_references_stop_sequence",
      ).on(table.stopSequence),
      idxTimetableNotesReferencesStopId: index(
        "idx_timetable_notes_references_stop_id",
      ).on(table.stopId),
      idxTimetableNotesReferencesTripId: index(
        "idx_timetable_notes_references_trip_id",
      ).on(table.tripId),
      idxTimetableNotesReferencesRouteId: index(
        "idx_timetable_notes_references_route_id",
      ).on(table.routeId),
      idxTimetableNotesReferencesTimetableId: index(
        "idx_timetable_notes_references_timetable_id",
      ).on(table.timetableId),
    };
  },
);

export const tripsDatedVehicleJourneys = sqliteTable(
  "trips_dated_vehicle_journeys",
  {
    tripId: numeric("trip_id").notNull(),
  ...
  (table) => {
    return {
      idxTripsDatedVehicleJourneysJourneyNumber: index(
        "idx_trips_dated_vehicle_journeys_journey_number",
      ).on(table.journeyNumber),
      idxTripsDatedVehicleJourneysOperatingDayDate: index(
        "idx_trips_dated_vehicle_journeys_operating_day_date",
      ).on(table.operatingDayDate),
      idxTripsDatedVehicleJourneysTripId: index(
        "idx_trips_dated_vehicle_journeys_trip_id",
      ).on(table.tripId),
    };
  },
);
...
export const directions = sqliteTable(
  ...
  (table) => {
    return {
      pk0: primaryKey({
        columns: [table.directionId, table.routeId],
        name: "directions_direction_id_route_id_pk",
      }),
    };
  },
);
...
export const boardAlight = sqliteTable(
  "board_alight",
  {
    tripId: numeric("trip_id").notNull(),
    stopId: numeric("stop_id").notNull(),
  ...
  (table) => {
    return {
      idxBoardAlightServiceDepartureTimestamp: index(
        "idx_board_alight_service_departure_timestamp",
      ).on(table.serviceDepartureTimestamp),
      idxBoardAlightServiceArrivalTimestamp: index(
        "idx_board_alight_service_arrival_timestamp",
      ).on(table.serviceArrivalTimestamp),
      idxBoardAlightServiceDate: index("idx_board_alight_service_date").on(
        table.serviceDate,
      ),
      idxBoardAlightRecordUse: index("idx_board_alight_record_use").on(
        table.recordUse,
      ),
      idxBoardAlightStopSequence: index("idx_board_alight_stop_sequence").on(
        table.stopSequence,
      ),
...
export const rideFeedInfo = sqliteTable(
  "ride_feed_info",
  {
  ...
  (table) => {
    return {
      idxRideFeedInfoGtfsFeedDate: index(
        "idx_ride_feed_info_gtfs_feed_date",
      ).on(table.gtfsFeedDate),
      idxRideFeedInfoRideEndDate: index("idx_ride_feed_info_ride_end_date").on(
        table.rideEndDate,
      ),
      idxRideFeedInfoRideStartDate: index(
        "idx_ride_feed_info_ride_start_date",
      ).on(table.rideStartDate),
    };
  },
);

export const riderTrip = sqliteTable(
  "rider_trip",
  {
  ...
  (table) => {
    return {
      idxRiderTripAlightingTimestamp: index(
        "idx_rider_trip_alighting_timestamp",
      ).on(table.alightingTimestamp),
      idxRiderTripBoardingTimestamp: index(
        "idx_rider_trip_boarding_timestamp",
      ).on(table.boardingTimestamp),
      idxRiderTripServiceDate: index("idx_rider_trip_service_date").on(
        table.serviceDate,
      ),
      idxRiderTripAlightingStopSequence: index(
        "idx_rider_trip_alighting_stop_sequence",
      ).on(table.alightingStopSequence),
      idxRiderTripAlightingStopId: index("idx_rider_trip_alighting_stop_id").on(
        table.alightingStopId,
      ),
      idxRiderTripBoardingStopSequence: index(
        "idx_rider_trip_boarding_stop_sequence",
      ).on(table.boardingStopSequence),
      idxRiderTripBoardingStopId: index("idx_rider_trip_boarding_stop_id").on(
        table.boardingStopId,
      ),
      idxRiderTripTripId: index("idx_rider_trip_trip_id").on(table.tripId),
      idxRiderTripAgencyId: index("idx_rider_trip_agency_id").on(
        table.agencyId,
      ),
    };
  },
...
export const ridership = sqliteTable(
  "ridership",
  {
  ...
  (table) => {
    return {
      idxRidershipDirectionId: index("idx_ridership_direction_id").on(
        table.directionId,
      ),
      ...
      idxRidershipServiceId: index("idx_ridership_service_id").on(
        table.serviceId,
      ),
      idxRidershipRidershipEndTimestamp: index(
        "idx_ridership_ridership_end_timestamp",
      ).on(table.ridershipEndTimestamp),
      idxRidershipRidershipStartTimestamp: index(
        "idx_ridership_ridership_start_timestamp",
      ).on(table.ridershipStartTimestamp),
      idxRidershipRidershipEndDate: index(
        "idx_ridership_ridership_end_date",
      ).on(table.ridershipEndDate),
      idxRidershipRidershipStartDate: index(
        "idx_ridership_ridership_start_date",
      ).on(table.ridershipStartDate),
    };
  },
);

export const tripCapacity = sqliteTable(
  "trip_capacity",
  {
    agencyId: numeric("agency_id"),
    tripId: numeric("trip_id"),
  ...
  (table) => {
    return {
      idxTripCapacityServiceDate: index("idx_trip_capacity_service_date").on(
        table.serviceDate,
      ),
      idxTripCapacityTripId: index("idx_trip_capacity_trip_id").on(
        table.tripId,
      ),
      idxTripCapacityAgencyId: index("idx_trip_capacity_agency_id").on(
        table.agencyId,
      ),
    };
  },
...
export const tripUpdates = sqliteTable(
  ...
  (table) => {
    return {
      idxTripUpdatesRouteId: index("idx_trip_updates_route_id").on(
        table.routeId,
      ),
...
export const stopTimeUpdates = sqliteTable(
  ...
  (table) => {
    return {
      idxStopTimeUpdatesStopId: index("idx_stop_time_updates_stop_id").on(
        table.stopId,
      ),
      idxStopTimeUpdatesRouteId: index("idx_stop_time_updates_route_id").on(
        table.routeId,
      ),
      idxStopTimeUpdatesTripId: index("idx_stop_time_updates_trip_id").on(
        table.tripId,
      ),
    };
  },
...
export const serviceAlertTargets = sqliteTable(
  "service_alert_targets",
  {
...
export const deadheadTimes = sqliteTable(
  "deadhead_times",
  {
    id: integer("id").primaryKey(),
...
export const deadheads = sqliteTable(
  "deadheads",
  {
...
export const runsPieces = sqliteTable(
  "runs_pieces",
  {
